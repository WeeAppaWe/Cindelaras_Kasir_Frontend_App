import { defineComponent, ref, reactive, computed, mergeProps, unref, withCtx, openBlock, createBlock, Fragment, renderList, createVNode, toDisplayString, createTextVNode, resolveDynamicComponent, isRef, createCommentVNode, useSSRContext } from 'vue';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const selectedIngredientTypeOption = computed(() => ingredientTypeOptions.find((option) => option.value === parameters.ingredientType) ?? ingredientTypeOptions[1]);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(currentView) === "input") {
        ...
        _push(`</div></div><div class="grid gap-3 sm:grid-cols-2"><div class="rounded-lg border bg-muted/30 p-3"><label for="spk-ingredient-type" class="mb-2 block text-xs font-semibold uppercase tracking-wide text-muted-foreground"> Jenis bahan </label>`);
        _push(ssrRenderComponent(unref(_sfc_main$2), {
          id: "spk-ingredient-type",
          modelValue: unref(parameters).ingredientType,
          "onUpdate:modelValue": ($event) => unref(parameters).ingredientType = $event,
          class: "w-full bg-background"
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<!--[-->`);
              ssrRenderList(ingredientTypeOptions, (option) => {
                _push2(`<option${ssrRenderAttr("value", option.value)}${_scopeId}>${ssrInterpolate(option.label)}</option>`);
              });
              _push2(`<!--]-->`);
            } else {
              return [
                (openBlock(), createBlock(Fragment, null, renderList(ingredientTypeOptions, (option) => {
        ...
        _push(`<p class="mt-1.5 text-xs text-muted-foreground">${ssrInterpolate(unref(selectedIngredientTypeOption).description)}</p></div><div class="rounded-lg border bg-muted/30 p-3"><label for="spk-supplier" class="mb-2 block text-xs font-semibold uppercase tracking-wide text-muted-foreground"> Pemasok </label>`);
        _push(ssrRenderComponent(unref(_sfc_main$2), {
          id: "spk-supplier",
...
//# sourceMappingURL=purchase-recommendations-CwdfDILx.mjs.map
