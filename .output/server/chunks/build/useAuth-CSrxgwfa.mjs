import { b as useCookie, u as useApiClient, a as apiEndpoints } from './api-endpoints-BXkjOpII.mjs';
import { d as useRuntimeConfig, n as navigateTo } from './server.mjs';
...
function useAuthApi() {
  ...
  function login(payload) {
    return requestAuthData(apiEndpoints.auth.login, payload, {
      auth: false
    });
  }
  function requestPasswordResetOtp(payload) {
    return requestAuthData(apiEndpoints.auth.forgotPasswordRequestOtp, payload, {
      auth: false
    });
  }
  function verifyPasswordResetOtp(payload) {
    return requestAuthData(apiEndpoints.auth.forgotPasswordVerifyOtp, payload, {
      auth: false
    });
  }
  function resetPassword(payload) {
    return requestAuthData(apiEndpoints.auth.forgotPasswordResetPassword, payload, {
      auth: false
    });
  }
  function logout() {
    return requestAuthData(apiEndpoints.auth.logout);
  }
  function requestAuthData(endpoint, body, options = {}) {
...
function useAuth() {
  ...
  const session = useCookie(authCookieName, {
    default: () => null,
    path: "/",
    sameSite: "lax"
  });
  ...
  async function login(payload) {
    ...
    try {
      ...
      const ttl = Math.max(result.token.expires_in, 1) * 1e3;
      const nextSession = {
  ...
  async function verifyPasswordResetCode(payload) {
    ...
    try {
      ...
      resetTokenExpiresAt.value = Date.now() + Math.max(result.expires_in ?? 1, 1) * 1e3;
      return true;
    } catch (error) {
      resetCodeVerified.value = false;
      resetToken.value = "";
...
function getDefaultAuthenticatedPath(role) {
  if (role === "admin") {
    return "/admin";
  }
  if (role === "cashier") {
...
export { getDefaultAuthenticatedPath as g, useAuth as u };
//# sourceMappingURL=useAuth-CSrxgwfa.mjs.map
